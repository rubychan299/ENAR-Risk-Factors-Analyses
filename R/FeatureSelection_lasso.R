...
lasso_rank <- function(outcome, train_data, weight){
  
  # Fit the lasso model
  y <- train_data[,outcome]
  x <- as.matrix(train_data[,vars])
  lasso_model <- cv.glmnet(x, y, family = "binomial", alpha = 0.5, weights = train_data[,weight])
  importance <- coef(lasso_model, s = lasso_model$lambda.min)
  rankimp <- arrange(importance,desc(MeanDecreaseGini))
  
  output <- list(lasso_model = lasso_model, rankimp = rankimp)
  return(output)
}

lasso_eval <- function(outcome, train_data, weight, test_data){
  
  # Fit the rf model
  model <- lasso_rank(outcome,train_data, weight)$lasso_model
...
lasso_rank_samp <- function(outcome, data_bootstrapped, weight){
  ...
  for(i in 1:length(data_bootstrapped)){
    ...
    fit <- lasso_rank_rank(outcome,train_data, weight)
    model[[i]] <- fit$lasso_model
...
### Pre 2013####
pre2013_bp_control_jnc7 <- lasso_eval("bp_control_jnc7",dat_hyp_final_pre_train, "svy_weight_mec",dat_hyp_final_pre_test)
