...
files_needed <- c("DEMO_L.xpt", "BMX_L.xpt", "BPXO_L.xpt", "INQ_L.xpt", "CBC_L.xpt", "HDL_L.xpt", "PBCD_L.xpt", "WHQ_L.xpt", "MCQ_L.xpt", "SMQ_L.xpt", "DIQ_L.xpt")
...
data <- data %>%
  ...
  select(
    SEQN, BMXBMI, INDFMMPI, LBDBANO, LBDHDD, LBDMONO, LBXBPB, LBXBSE, LBXRBCSI, LBXWBCSI, 
...
# Does it match with str(dat_hyp_2013_final[[1]])?
common_vars <- intersect(names(data), names(dat_hyp_2013_final[[1]]))
mismatches <- sapply(common_vars, function(var) {
  class(data[[var]])[1] != class(dat_hyp_2013_final[[1]][[var]])[1]
})
mismatched_vars <- common_vars[mismatches]
print(mismatched_vars)
